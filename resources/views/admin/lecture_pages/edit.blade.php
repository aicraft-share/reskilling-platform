<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.lecture-pages.index') }}" class="text-sm font-medium text-slate-500 hover:text-slate-700 transition">一覧に戻る</a>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto bg-white overflow-hidden shadow-sm rounded-xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
        <div class="p-8">
            <form method="POST" action="{{ route('admin.lecture-pages.update', $lecturePage) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="course_id" name="course_id" class="block mt-1 w-full border-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm" required>
                            ...
                            @foreach($courses as $course)
                                <option value="{{ $course->id }}" {{ old('course_id', $lecturePage->course_id) == $course->id ? 'selected' : '' }}>
                                    {{ $course->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="section_name" class="block mt-1 w-full" type="text" name="section_name"
                            :value="old('section_name', $lecturePage->section_name)" placeholder="例：はじめに、実践、応用など" />
                ...
                <div class="mt-6">
                    ...
                    @if($lecturePage->thumbnail_path)
                        <div class="mt-2 mb-4">
                            <img src="{{ asset('storage/' . $lecturePage->thumbnail_path) }}" alt="Current Thumbnail" class="h-32 w-auto rounded-lg border border-slate-200 shadow-sm">
                            <p class="text-[10px] text-slate-500 mt-1">現在のサムネイル</p>
                        </div>
                    @endif
                    ...
                    <p class="mt-1 text-xs text-slate-500">※ 新しい画像をアップロードすると上書きされます。推奨: 16:9</p>
                ...
                <div class="mt-6">
                    ...
                    <x-text-input id="youtube_url" class="block mt-1 w-full" type="url" name="youtube_url"
                        :value="old('youtube_url', $lecturePage->youtube_url)"
